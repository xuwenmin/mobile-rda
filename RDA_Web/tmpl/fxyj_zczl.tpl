<!-- 风险预警--资产质量 -->
 <div class="head">
                     <img from="fxyj" class="pre" src="images/pre.png" alt="">
                     <h1>风险预警</h1>
                     <a href="#" class="next"></a>
                 </div>
                 <div class="content">
                     <ul class="content_list">
                         <li>
                             <a href="#" class="desc">资产质量<p class="list_next list_topright"></p></a>
                         </li>
                         <li rel="li1"  class="hassub">
                             <a href="#" class="desc">
                                 <img src="images/icon1.png" alt="" class="icon">
                                 资产利用效率<div class="list_next list_topright">
                                 <img src="images/a_down.png" alt="" class="imgdown">
                                 <img src="images/a_top.png" alt="" class="imgtop">
                             </div></a>
                         </li>
                         <li  name="li1" class="subli">
                             <a href="#" class="desc">
                                 货币资金<div class="list_next list_topright">

                             </div></a>
                         </li>
                         <li rel="li2" class="hassub">
                             <a href="#" class="desc">
                                 <img src="images/icon1.png" alt="" class="icon">
                                 现金含量<div class="list_next list_topright">
                                 <img src="images/a_down.png" alt="" class="imgdown">
                                 <img src="images/a_top.png" alt="" class="imgtop">
                             </div></a>
                         </li>
                         <li name="li2" class="subli">
                             <a href="#" class="desc">
                                 长期投资<div class="list_next list_topright">

                             </div></a>
                         </li>
                     </ul>
                 </div>