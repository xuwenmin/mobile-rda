<!-- 融资管理--发布融资信息--第一步 -->
                <div class="head">
                     <img from="rzgl" class="pre" src="images/pre.png" alt="">
                     <h1>融资管理</h1>
                     <a href="#" class="next"></a>
                 </div>
                 <div class="content">
                     <!-- 内容头部 -->
                     <div class="content_title">
                         <p>第一步、填写项目信息</p>
                     </div>
                     <div class="contentform nopdform" id="qm_wrapper">
                         <div class="qm_scroll">
                              <div id="rzgl_rzxx_main">
                                
                              
                                 <div class="inputline">
                                     <label>项目名称:</label>
                                     <div class="divparent">
                                         <input id="r_projectname" class="qm_input" type="search"/>
                                     </div>
                                 </div>
                                 <div class="inputline ">
                                     <label  class="ver_top" >融资类型:</label>
                                     <div class="divparent sjinputlinebottom ver_top">
                                         <input class="qm_input qm_input_p40" readonly="true"  id="rzgl_rzlx" type="input"/>
                                         <!--融资类型选项-->
                                         <div class="extend hide" rel="rzgl_rzlx" id="rzgl_rzlx_item" >
                                           
                                         </div>
                                     </div>
                                 </div>
                                 <div class="inputline">
                                     <label>所属行业:</label>
                                     <div class="divparent sjinputlineright">
                                         <input id="rzgl_sshy" class="qm_input qm_input_p40" readonly="true" type="input"/>
                                     </div>
                                 </div>
                                 <div class="inputline txtlign_left">
                                     <label>融资时长:</label>
                                     <div class="divparent sjinputlinebottom divshort50">
                                         <input id="rzgl_sc" class="qm_input qm_input_p40"  readonly="true" type="input"/>
                                         <ul id="rzgl_rzsc_item" for="rzgl_sc" class="ul_select hide">
                                          <!--  <li>长期</li>
                                          <li>短期</li>
                                          <li>其它</li> -->
                                         </ul>
                                     </div>
                                 </div>
                                 <div class="inputline">
                                     <label>融资金额:</label>
                                     <div class="divparent sjinputlinebottom divshort24">
                                         <input id="rzgl_rmb" class="qm_input qm_input_p40" readonly="true" type="input" />
                                         <ul id="rzgl_rzbz_item" for="rzgl_rmb" class="ul_select hide">
                                         <!--   <li>人民币</li>
                                         <li>美元</li>
                                         <li>其它</li> -->
                                         </ul>
                                     </div>
                                     <div class="divparent divshort19">
                                         <input id="r_moneystart" class="qm_input inputmoney" type="input" value="128200"/>
                                     </div>
                                     <div class="divparent divshort5">
                                         至
                                     </div>
                                     <div class="divparent divshort30">
                                         <input id="r_moneyend" class="qm_input inputmoney" type="input" value="128200"/>
                                     </div>
                                 </div>
                                 <div class="inputline hide">
                                     <label class="ver_top">项目简介:</label>
                                     <div class="divparent ver_top">
                                         <textarea class="qm_input qm_input80h" id="rzgl_pinfo" >11</textarea>
                                     </div>
                                 </div>
                                 <div class="inputline hide">
                                     <label>商业计划书:</label>
                                     <div class="divparent">
                                         <input class="qm_input" type="search" />
                                     </div>
                                 </div>
                                 <div class="inputline">
                                     <!-- <a href="#" class="button" >保存</a> -->
                                     <a href="#" class="button" id="rzgl_next" >下一步</a>
                                 </div>
                              </div>
                              <ul class="content_list hide" id="rzgl_rzxx_sub">
                             </ul>
                             <a href="#" class="a_button" style="display:none" id="sel_rzsshy">确定</a>
                        </div>
                     </div>

                 </div>
<div class="popwin" id="pop1">
    <p class="pop_red pop_red">项目简介未提交</p>
    <p>请联系公司相关人员通过电脑客户端上传后再发布信息</p>
    <a href="#" class="pop_button pop_return">返回</a>
</div>

